...
drop table if exists public.activity_day_jouriors;
...
(
	...
	phone text not null check (phone ~ '^\+7[\s()-]*\d{3}[\s()-]*\d{3}[\s()-]*\d{2}[\s()-]*\d{2}$'),
...
create table if not exists public.activity_day_jouriors
(
	...
	foreign key (jury_id) references public.jury (user_id)
		on delete cascade,
    foreign key (activity_day_id) references public.activity_day (id)
        on delete cascade
);